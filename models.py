...
class HybridCodeNetwork(nn.Module):
    def __init__(self, bow_size, vocab_size, embd_size, hidden_size, action_size, context_size, pre_embd_w=None):
        super(HybridCodeNetwork, self).__init__()
        self.embd_size = embd_size
        self.hidden_size = hidden_size
        self.embedding = WordEmbedding(vocab_size, embd_size, pre_embd_w)
        lstm_in_dim = embd_size + bow_size + action_size + context_size + 1 # + 1 (Unknown vocab tag)
    ...
    def forward(self, uttr, context, bow, prev, act_filter):
        ...
        # why is it taking embedding mean
